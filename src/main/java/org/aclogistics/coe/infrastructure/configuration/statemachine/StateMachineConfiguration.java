...
public class StateMachineConfiguration extends StateMachineConfigurerAdapter<Status, Event> {
    ...
    public void configure(StateMachineTransitionConfigurer<Status, Event> transitions) throws Exception {
        transitions
            ...
            .and()
            .withExternal()
                .source(Status.REQUESTED).target(Status.PENDING_LINE_MANAGER_APPROVAL)
            ...
            .and()
            .withExternal()
                .source(Status.PENDING_LINE_MANAGER_APPROVAL).target(Status.LINE_MANAGER_APPROVED)
                .guard(lineManagerApprovalGuard)
                .event(Event.LINE_MANAGER_APPROVE)
            .and()
            .withExternal()
                .source(Status.PENDING_LINE_MANAGER_APPROVAL).target(Status.REJECTED)
